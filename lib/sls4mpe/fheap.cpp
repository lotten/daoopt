...
printf("alloc, ");
    ...
    h->max_trees = (int) ceil(1.0 + 1.44 * log(max_nodes+0.0)/log(2.0));
...
/* fh_delete_min() - returns the vertex number of the minimum node of the heap pointed to by h.
 * Does NOT change the heap!
 * Extension by Frank Hutter. Hope it works ;)
 */
int fh_inspect(fheap_t *h){
    fheap_node_t *min_node, *next;
    double k, k2;
    int r, v;
