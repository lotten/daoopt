...
bool Factor::mxCheckValid(const mxArray* M) {
	return t_.mxCheckValid(mxGetField(M,0,"t")) && v_.mxCheckValid(mxGetField(M,0,"v"));
}
...
vector<Factor> Factor::readUai10(std::istream& is) {
  ...
  for (size_t i=0;i<ncliques;i++) {
    ...
    for (size_t j=0;j<csize;j++) { 
      is>>v; Var V(v+1,dims[v]);    // 0 vs 1 based !!!
...
//inline std::ostream& operator<<( std::ostream& out, const mex::Factor& F) { F.display(out); return out; }
